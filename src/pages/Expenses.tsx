...
const Expenses: React.FC = () => {
  ...
  // FIX: Helper for optimistic update to instantly remove from pending list
  const updateExpenseStatusLocally = (id: number, newStatus: ExpenseItem['status']) => {
    setExpenses(prevExpenses =>
      prevExpenses.map(expense =>
        expense.id === id ? {
          ...expense,
          status: newStatus,
          // Set reviewed_on to current time, synchronized with backend fix
  ...
  const handleApprove = async (id: number) => {
    try {
      ...
      // FIX: Optimistic Update (instant visual removal)
      updateExpenseStatusLocally(id, 'APPROVED');
  ...
  const handleReject = async (id: number) => {
    try {
      ...
      // FIX: Optimistic Update (instant visual removal)
      updateExpenseStatusLocally(id, 'REJECTED');
  ...
  const handleCreate = async () => {
    console.log('handleCreate called. Form:', form);
    // Try to get current user ID from token if not provided in form
    let employeeId = form.employee ? Number(form.employee) : null;
    ...
    if (!employeeId) {
      try {
        ...
        if (token) {
          ...
          employeeId = payload.user_id || payload.id;
        }
      } catch (e) {
    ...
    if (!form.title || !form.amount || !form.category || !employeeId) {
      toast({ title: 'Error', description: 'Please fill in all fields. Employee ID is required if not logged in.', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      toast({ title: 'Error', description: 'Failed to create expense. Please check inputs.', variant: 'destructive' });
    }
  };
        ...
        <Dialog open={createOpen} onOpenChange={setCreateOpen}>
          <DialogContent>
            ...
            <div className="grid gap-2">
              ...
              <Input placeholder="Employee ID (Leave empty for current user)" value={form.employee} onChange={(e) => setForm(f => ({ ...f, employee: e.target.value }))} />
            </div>
            <DialogFooter>
